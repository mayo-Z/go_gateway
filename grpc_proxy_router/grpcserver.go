...
func GrpcServerRun() {
	...
	for _, serviceItem := range serviceList {
		...
		go func(serviceDetail *dao.ServiceDetail) {
			...
			if err != nil {
				log.Fatalf(" [ERROR] GetLoadBalancer%v err:%v\n", addr, err)
				return
			}
			lis, err := net.Listen("tcp", addr)
			if err != nil {
				log.Fatalf(" [ERROR] GrpcListen %v err:%v\n", addr, err)
			}
			...
			if err := s.Serve(lis); err != nil &&
				err != tcp_server.ErrServerClosed {
				log.Fatalf(" [ERROR] grpc_proxy_run:%v err:%v\n", addr, err)
			}
		}(tempItem)
	}
}
...
func GrpcServerStop() {
	for _, grpcServer := range grpcServerList {
		...
		log.Fatalf(" [INFO] grpc_proxy_stop %v stopped\n", grpcServer.Addr)
	}
}
