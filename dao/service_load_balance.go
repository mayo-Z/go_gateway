...
//负载均衡管理器和构建单例负载均衡！！！
//一个服务一个LoadBalance
//服务较多时使用ServiceName查找LoadBalanceMap
//服务较少时遍历LoadBalanceSlice
...
func (lbr *LoadBalancer) GetLoadBalancer(service *ServiceDetail) (load_balance.LoadBalance, error) {
	...
	lb := load_balance.LoadBanlanceFactorWithConf(load_balance.
		LbType(service.LoadBalance.RoundType), mConf)
